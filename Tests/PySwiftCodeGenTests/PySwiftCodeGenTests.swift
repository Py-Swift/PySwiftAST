...
@Test func testFStringWithQuotesInExpression() async throws {
    let code = "x = f\"{', '.join(fields)}\""
    print("\nTesting f-string with quotes in expression:")
    print("Input: \(code)")
    print("Input bytes: \(code.utf8.map { String(format: "%02X", $0) }.joined(separator: " "))")
    
    let ast = try parsePython(code)
    let generated = generatePythonCode(from: ast)
    print("Generated: \(generated)")
    print("Generated bytes: \(generated.utf8.map { String(format: "%02X", $0) }.joined(separator: " "))")
...
@Test func testDatabaseORMRoundTrip() async throws {
    ...
    print("\nTesting: database_orm.py (Round-Trip)")
    print("Source lines: \(source.split(separator: "\n").count)")
